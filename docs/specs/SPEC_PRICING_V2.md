...
- **Do not say "unlimited" for a metered or billed dimension.** Bytes, model runs, overage — anything the meter counts. The whole pivot is about honest metering; "unlimited" in a volume context undoes that. For a **non-metered** dimension the word is permitted only while **no finite ceiling is enforced**, and you establish that by reading the enforcing row, not by reading this spec. **A sentinel is not a promise.**

  > 🔧 **Revised 2026-09-02 (landing#368), and the revision is a narrowing, not a relaxation.** This bullet used to read *"Do not say 'unlimited.' **Anywhere. Not on any tier, not for any dimension.**"* — which contradicted §2.1's own tier table and `/pricing/`, both of which print *Unlimited* for schedules. `tests/pricing-copy-rules.test.ts` therefore enforced every other word in this section and deliberately **not** this one, recording in its header that "a test is not the place to pick a side."
  >
  > The side is now pickable, because the missing input arrived: the clause's own stated reason is *"'unlimited' in a **volume** context undoes that"* — it was written about the metered dimension and over-generalised to every dimension. Schedules are neither metered nor billed, so the blanket ban was wider than its own rationale. Narrowing it to metered dimensions removes the contradiction without weakening the thing §4.3 exists to protect.
  >
  > ⚠️ **That did not make the copy true at the time.** `/pricing/`, the homepage and `/features/volume-pricing/` all say *"Unlimited schedules"* on Pro and Enterprise. Measured off the `plans` table on the serving box **2026-09-02T09:31Z**: `max_schedules` was **9999** on all four paid rows, and `BillingService.check_schedule_quota` **raised** `QuotaExceededError` at `len(schedules) >= plan.max_schedules` — no overage path, no flag in front of it. So the enforced ceiling was finite and it hard-blocked.
  >
  > 🟢 **RESOLVED — re-measured on the serving box 2026-09-07 (Growth, landing#396). The published claim is now TRUE.** core#928 and cloud#151 both shipped: `max_schedules` is **NULL** on `pro-monthly`, `pro-annual`, `enterprise-monthly` and `enterprise-annual` (`max_schedules IS NULL` → `t` on all four), and `check_schedule_quota` now opens `if plan is None or plan.max_schedules is None: return`, with a docstring citing this decision by name. Free is unchanged at 2. **This is option (c) completing exactly as designed** — the page was left standing and the row moved to meet it.
  >
  > 🚨 **Do not "correct" the page on the strength of the paragraph above.** For five days three separate documents — this spec, `docs/GROWTH_RULES.md` and the header of `tests/pricing-copy-rules.test.ts` — told a reader that live copy was false, while the enforcing row was on its way to making it true. Under option (c) that window is *structural*, not accidental: the page is deliberately false first and true later. **A dated measurement is evidence about its date and nothing else — re-read the row before acting on a note that says a claim is false, exactly as you would before publishing it.**
  >
  > **The page is deliberately NOT being changed, and the reason is the founder's 2026-08-31 pricing decision (option (c)): make the published page true, do not rewrite the page — its exact numbers are the acceptance criteria.** Confirmed by Product on landing#396: *"Unlimited" stays; Engineering makes it true.* Writing a finite number here would publish an implementation accident as a product boundary and retreat from a promise we intend to keep. Two sentinels for "unlimited" (`9999` observed, `999999` in migration `o4k1l2m3n5h6`) are the actual defect. Routed on **core#928**; nobody is affected meanwhile (0 subscriptions, and the observed ceiling is not a shape any customer has).
  >
  > 🚨 **CORRECTION, same day — the fix is NOT one data-only change, and the precedent I cited for it does not exist.** This block first said *"`max_api_keys` already establishes NULL = uncapped as this schema's convention, and cloud reads it that way; `max_schedules` should join it."* Product measured both halves and both are false: **`max_api_keys` has zero readers in `datanika-cloud`**, so "NULL means uncapped" is a convention true of no code, and **`check_schedule_quota` has no `None` guard** — so NULLing the column first gives `int >= None`, a **TypeError on the first schedule a paid customer creates**. Filed as **cloud#151: the reader ships before the migration.**
  >
  > 🔑 **The lesson is the one this section already teaches, turned on myself.** I read *"`max_api_keys` is NULL on every row, which cloud reads as uncapped"* in a handoff and repeated it as a property of the code. It is a property of a **sentence**. The rule below says a published claim must be bound to what production *enforces*; a **routing** claim has to be bound to what the code actually *reads*, and neither is established by a document that asserts it. Grep for the reader.
  >
  > 🔑 **The general form, which outlives this instance:** *the out-of-band row creator sets the columns that existed when it was written, so every column a later migration adds falls to its `server_default` on paid rows* (core#928). **Every absolute word on the pricing page — "unlimited", "all", "any", "never" — is a claim about a row, and the row is the only thing that can settle it.**
