...
- **Do not say "unlimited" for a metered or billed dimension.** Bytes, model runs, overage — anything the meter counts. The whole pivot is about honest metering; "unlimited" in a volume context undoes that. For a **non-metered** dimension the word is permitted only while **no finite ceiling is enforced**, and you establish that by reading the enforcing row, not by reading this spec. **A sentinel is not a promise.**
  ...
  >
  > **The page is deliberately NOT being changed, and the reason is the founder's 2026-08-31 pricing decision (option (c)): make the published page true, do not rewrite the page — its exact numbers are the acceptance criteria.** Writing "9,999 schedules" would publish an implementation accident as a product boundary and retreat from a promise that one data-only change keeps. `max_api_keys` already establishes **NULL = uncapped** as this schema's convention, and cloud reads it that way; `max_schedules` should join it. Two sentinels for "unlimited" (`9999` observed, `999999` in migration `o4k1l2m3n5h6`) plus a third convention meaning the same thing is the actual defect. Routed on **core#928**; nobody is affected meanwhile (0 subscriptions, and 9,999 schedules on one org is not a shape any customer has).
  >
