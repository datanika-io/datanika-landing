...
- **Do not say "unlimited" for a metered or billed dimension.** Bytes, model runs, overage — anything the meter counts. The whole pivot is about honest metering; "unlimited" in a volume context undoes that. For a **non-metered** dimension the word is permitted only while **no finite ceiling is enforced**, and you establish that by reading the enforcing row, not by reading this spec. **A sentinel is not a promise.**
  ...
  >
  > ⚠️ **That does not make today's copy true.** `/pricing/`, the homepage and `/features/volume-pricing/` all say *"Unlimited schedules"* on Pro and Enterprise. Measured off the `plans` table on the serving box **2026-09-02T09:31Z**: `max_schedules` is **9999** on all four paid rows, and `BillingService.check_schedule_quota` **raises** `QuotaExceededError` at `len(schedules) >= plan.max_schedules` — no overage path, no flag in front of it. So the enforced ceiling is finite and it hard-blocks.
  >
