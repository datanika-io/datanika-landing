...
describe("verified_date carries its own semantics at the declaration", () => {
  ...
  it("every guide's verified_by is a member of the vocabulary", () => {
    ...
    // did the walk). `qa-ui` joined on 2026-09-15 with the openapi sign-off (landing#572), and
    // `growth-ui` the same day with the mysql walk (landing#395).
    const KNOWN = ["draft-pending-verification", "verification-blocked", "product-ui", "qa-ui", "growth-ui"];
    const guides = readdirSync(GUIDES).filter((f) => f.endsWith(".md"));
    const stray: string[] = [];
