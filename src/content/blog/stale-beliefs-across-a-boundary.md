...
description: "Our billing gate correctly refuses to charge for a cancelled run. Its only caller passes the word success as a literal, from every path. Each side had written down an assumption about the other, both assumptions were reasonable, and neither was enforced — so the gate defends a case it can never be handed."
date: 2026-09-25
...
The only caller passes `status="success"` as a string literal.
...
> That worked only because of a property of *core*, in another repository: the completion events are announced solely from success paths. **Nothing here enforced or recorded that assumption.**
...
> Its stated fix was "announce the completion event with `status="failed"`" — which reads as the obvious change, since the notification handlers already branch on exactly that. **Implemented literally, every failed run would have been metered as a successful one.**

An issue proposed a change that looked obviously right by analogy with neighbouring code. Had someone implemented exactly what the ticket said, the billing gate would have been handed `"failed"` on the event it *does* meter, and the ticket would have closed green. It was avoided by shipping a separate event instead — a decision that reads, in the diff, as slightly more work for no visible benefit.
...
The durable fix is not "make the caller pass the real status", though we did that. It is to stop the assumption living in one repository's docstring:
