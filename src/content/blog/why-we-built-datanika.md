...
A pipeline that would be four tools and a hundred lines of glue becomes: add a source connection, add a destination, pick tables, write a model, set a schedule. There are [36 connectors](/connectors) — 31 sources and 9 destinations, overlapping on the four databases that do both — and every one of them is available on every plan, including the free one. We gate on volume, not on capability, because a connector you can't use is not a feature.
