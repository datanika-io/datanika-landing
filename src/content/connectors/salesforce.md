...
4. Because Salesforce is a SaaS source, the form shows **Select endpoints to load** — a checkbox per resource, **all ticked by default**. For Salesforce the list is `Account`, `Contact`, `Opportunity`, `Lead`, `Campaign`, `Case`. Untick anything you do not want; each ticked endpoint becomes its own table in the destination.
