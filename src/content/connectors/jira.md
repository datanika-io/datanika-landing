...
4. Because Jira is a SaaS source, the form shows **Select endpoints to load** — a checkbox per resource, **all ticked by default**. For Jira the list is `issues`, `users`, `workflows`, `projects`. Untick anything you do not want; each ticked endpoint becomes its own table in the destination.
