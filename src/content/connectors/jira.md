...
2. Fill in **Upload name** (letters, digits and spaces — anything else is stripped as you type, so `jira-daily-sync` becomes `jiradailysync`) and an optional **Description**.
...
3. When it finishes, open **Models** (`/models`) and browse the landed tables. The upload lands them in a schema **derived from the upload's name**: spaces become underscores and the whole thing is lower-cased. So `jiradailysync` creates schema `jiradailysync`, and an upload named `Jira Daily Sync` creates schema `jira_daily_sync` — worth knowing, since the name field accepts spaces. dlt also creates its own `_dlt_loads` / `_dlt_pipeline_state` / `_dlt_version` bookkeeping tables in that schema, but **Models does not list them** — seeing only your own tables there is correct, not a partial load. There is no target-schema field to choose.
...
>
> Tell the two apart before assuming a bug: a table that is **missing** means that endpoint returned no records; a table that **exists but is short** means rows were dropped. Checking the count in Jira itself is the fastest way to settle it.
