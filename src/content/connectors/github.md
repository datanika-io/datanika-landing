...
3. Fill in the form:
   ...
   - **Repositories** — comma-separated `owner/repo` pairs (e.g., `datanika-io/datanika-core, datanika-io/datanika-landing`). Or a single `owner/*` pattern to sync all repos in an org.
   - **Use GraphQL backend** — leave unchecked. The REST backend is more compatible and covers every resource this guide uses. Flip to GraphQL only if you're running into rate limits on very large orgs.
4. Click **Test connection**. Datanika calls `GET /user` and `GET /rate_limit` to verify the token and checks access to each specified repo. You should see a green ✅ within a few seconds.
5. Click **Save**.
