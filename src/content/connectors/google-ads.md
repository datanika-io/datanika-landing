...
4. Because Google Ads is a SaaS source, the form shows **Select endpoints to load** — a checkbox per resource, **all ticked by default**. For Google Ads the list is `customers`, `campaigns`, `ad_groups`, `ads`. Each endpoint becomes its own table in the destination.
...
> **⚠️ Unticking does not currently narrow the load.** The selection is stored as `endpoints` in the upload config, and the Google Ads loader does not read it — it pulls its full default resource set regardless ([core#532](https://github.com/datanika-io/datanika-core/issues/532)). Nothing fails; you simply get every table rather than the subset you picked. Drop what you do not need in a dbt model downstream until this is wired up.
