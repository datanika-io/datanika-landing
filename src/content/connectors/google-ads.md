...
3. When it finishes, open **Models** (`/models`) and browse the landed table. The upload lands it in a schema **derived from the upload's name**: spaces become underscores and the whole thing is lower-cased. So `googleadsdailysync` creates schema `googleadsdailysync`, and an upload named `Google Ads Daily Sync` creates schema `google_ads_daily_sync` — worth knowing, since the name field accepts spaces. dlt also creates its own `_dlt_loads` / `_dlt_pipeline_state` / `_dlt_version` bookkeeping tables in that schema, but **Models does not list them** — seeing only your own tables there is correct, not a partial load. There is no target-schema field to choose.
