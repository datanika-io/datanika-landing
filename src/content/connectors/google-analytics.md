...
4. Because Google Analytics is a SaaS source, the form shows **Select endpoints to load** — a checkbox per resource, **all ticked by default**. For Google Analytics the list is `report`. Each endpoint becomes its own table in the destination.
...
> **⚠️ Unticking does not currently narrow the load.** The selection is stored as `endpoints` in the upload config, and the Google Analytics loader does not read it — it pulls its full default resource set regardless ([core#532](https://github.com/datanika-io/datanika-core/issues/532)). Nothing fails; you simply get every table rather than the subset you picked. Drop what you do not need in a dbt model downstream until this is wired up.
...
3. When it finishes, open **Catalog** and browse the landed tables. The upload lands them in a schema **named after the upload** — `ga4dailysync` creates schema `ga4dailysync` in the destination, next to dlt's `_dlt_loads` / `_dlt_pipeline_state` / `_dlt_version` bookkeeping tables. There is no target-schema field to choose.
